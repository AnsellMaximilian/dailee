...
export default function SignUp({setMessage}: Props) {
    ...
    const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .then((userRef) => {
                firestore.collection('profiles').doc(userRef.user?.uid).set({
                    fullName,
                    goals,
                }).then(() => setMessage({type: 'success', content: `Successfully signed in as ${fullName}`}))
            })
    ...
    return (
        <Container maxWidth="xs">
            <form onSubmit={handleSubmit}>
                ...
                <TextField required fullWidth type="text" margin="normal" variant="outlined" label="Full Name" name="fullName" value={fullName} onChange={handleChange}/>
                <TextField fullWidth type="text" margin="normal" variant="outlined" label="Goals" name="goals" value={goals} onChange={handleChange}/>
